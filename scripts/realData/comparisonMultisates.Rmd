---
title: "LwGT"
author: "Yang Xiao"
date: "2025-03-26"
output: html_document
---
```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

library(cmdstanr)
library(bayesplot)

```

# Environment
```{r}
path_proj = here::here()
path_source = file.path(path_proj, "source")

source(file.path(path_source, "functions", "dengue_tracker_functions.R"))
source(file.path(path_source, "functions", "tidy_dengueBR.R"))
source(file.path(path_source, "functions", "fit_function.R"))
source(file.path(path_source, "functions", "metrics.R"))
aweek::set_week_start("Sunday")

path_source_nowcasting <- file.path(dirname(path_proj), "nowcasting", "source")
source(file.path(path_source_nowcasting, "simulation", "simulations_functions_final.R"))
source(file.path(path_source_nowcasting, "functions", "prior_function.R"))
source(file.path(path_source_nowcasting, "functions", "fit_function.R"))
source(file.path(path_source_nowcasting, "functions", "plot_function.R"))

path_source_denguetracker_data <- file.path(dirname(path_proj), "dengue-tracker/data/weekly_data/")
path_source_denguetracker_infodengue <- file.path(path_source_denguetracker_data, "infodengue")
path_source_denguetracker_GT <- file.path(path_source_denguetracker_data, "gtrends")

posterior_draws_path = file.path(dirname(path_proj),"draws")
```
# Parameter settings
```{r}
delay_D <- 15
iter_sampling <- 5000
iter_warmup <- 2000
window_length <- 10
step_size <- 9

ew_start <- "202410"
ew_end <- "202452"

# no state ES
 brazil_ufs <- c(
   "AC", "AL", "AP", "AM", "BA", "CE", "DF", "GO",
   "MA", "MT", "MS", "MG", "PA", "PB", "PR", "PE", "PI",
   "RJ", "RN", "RS", "RO", "RR", "SC", "SP", "SE", "TO" # "BR"
 )

# Test RJ (0.977), SP (0.972) (big cities), AP(0.7395), "RR" (0.5887), CE (0.8938)
brazil_ufs <- c("RJ", "SP", "AP", "RR")
brazil_ufs <- c("AC")
```


# Model ARABM
```{r}

ARABM_result <- list()
ARABM_result_out <- list()
time_start_ARABM <- Sys.time()
for (i in 1:length(brazil_ufs)) {
  print(paste0("Now is the state ", brazil_ufs[i],"."))
  ARABM_result[[i]] <- run_moving_window(
    root_dir_infodengue = path_source_denguetracker_infodengue,
    root_dir_GT = path_source_denguetracker_GT,
    state = brazil_ufs[i],
    full_start = ew_start,
    full_end = ew_end,
    window_length = window_length,
    step_size = step_size,
    D = delay_D,
    hypers = hypers_q(phi_ref = 0.2, D_ref = 15, type = "exponential",
                      alpha_phi = 1.4, sd_log_b = 1, delay_seq = 0:15),
    model_file = file.path(path_source_nowcasting, "models", "b-ou.stan"),
    model_name = "ARABM",
    mode = "fixed",
    iter_sampling = iter_sampling,
    iter_warmup = iter_warmup,
    chains = 3,
    posterior_draws_path = file.path(posterior_draws_path, "ARABM")
  )
  ARABM_result_out[[i]] <- extract_N_summary(ARABM_result[[i]], num_N = 1)
}
time_end_ARABM <- Sys.time()
time_end_ARABM - time_start_ARABM

```

# Model ARABMwGT
```{r}
ARABMwGT_result <- list()
ARABMwGT_result_out <- list()
time_start_ARABMwGT <- Sys.time()
for (i in 1:length(brazil_ufs)) {
  ç[[i]] <- run_moving_window(
    root_dir_infodengue = path_source_denguetracker_infodengue,
    root_dir_GT = path_source_denguetracker_GT,
    state = brazil_ufs[i],
    full_start = ew_start,
    full_end = ew_end,
    window_length = window_length,
    step_size = step_size,
    D = delay_D,
    hypers = hypers_q(phi_ref = 0.2, D_ref = 15, type = "exponential",
                      alpha_phi = 1.4, sd_log_b = 1, delay_seq = 0:15),
    model_file = file.path(path_source, "models", "b-ou-GT-combined.stan"),
    model_name = "ARABMwGT",
    mode = "fixed",
    iter_sampling = iter_sampling,
    iter_warmup = iter_warmup,
    chains = 3,
    posterior_draws_path = file.path(posterior_draws_path, "ARABMwGT")
  )
  ARABMwGT_result_out[[i]] <- extract_N_summary(ARABMwGT_result[[i]], num_N = 1)
}
time_end_ARABMwGT <- Sys.time()

time_end_ARABMwGT - time_start_ARABMwGT

```

```{r}
# data_file <- "E:/Project3_draws"

#save
# save(ARABM_result, file = file.path(data_file,  "ARABM_result_raw.RData"))
# save(ARABM_result_out, file = file.path(data_file,  "ARABM_result.RData"))
# 
# save(ARABMwGT_result, file = file.path(data_file,  "ARABMwGT_result_raw.RData"))
# save(ARABMwGT_result_out, file = file.path(data_file, "ARABMwGT_result.RData"))

load(file.path(path_proj, "data", "results", "ARABM_result_out.RData"))
load(file.path(path_proj, "data", "results", "ARABMwGT_result_out.RData"))
```

# Baseline data
```{r, warning=FALSE}
baseline_list <- list()
for (i in 1:length(brazil_ufs)) {
   baseline_list[[i]] <- get_baseline_data(
     root_dir = path_source_denguetracker_data, # root file for data,
     states = brazil_ufs[i],
     start = ew_start,
     end = ew_end,
     last_n = 10,
     time_unit = "week"
   )
 }

# save(baseline_list, file = file.path(path_proj, "data", "results", "baseline_list.RData"))
#load(file.path(path_proj, "data", "results", "baseline_list.RData"))
baseline_list
```

```{r}
# wrong name, change name
ç <- map(
  ARABMwGT_result_out,
  ~ rename(.x,
    ARABMwGT       = ARABM,
    ARABMwGT_lwr   = ARABM_lwr,
    ARABMwGT_upr   = ARABM_upr
  )
)
save(ARABMwGT_result_out, file = file.path(path_proj, "data", "results", "ARABMwGT_result_out.RData"))
```


```{r}

result_out_list <- list()
for (i in 1:length(brazil_ufs)) {
  # baseline_list[[i]] <- get_baseline_data(
  #   root_dir = path_source_denguetracker_data, # root file for data,
  #   states = test_states[i],
  #   start = ew_start,
  #   end = ew_end,
  #   last_n = 8
  # )
  result_out_list[[i]] <- baseline_list[[i]] %>%
    left_join(
      ARABM_result_out[[i]] %>% select(ew_now, ew, ARABM, ARABM_lwr, ARABM_upr),
      by = c("ew_now", "ew")
    ) %>%
    left_join(
      ARABMwGT_result_out[[i]] %>% select(ew_now, ew, ARABMwGT, ARABMwGT_lwr, ARABMwGT_upr),
      by = c("ew_now", "ew")
    )
  
  result_out_list[[i]] <- add_actual_cases(result_out_list[[i]],
                                         root_dir_infodengue = path_source_denguetracker_infodengue,
                                         brazil_ufs[i],
                                         202512)
  result_out_list[[i]] <- result_out_list[[i]] %>%
    filter(!is.na(ARABM))
}



```

```{r}
print(result_out_list[[12]])
read.csv(file.path(path_source_denguetracker_GT,202425, paste0("MG","_trends.csv")), 
               stringsAsFactors = FALSE, skip = 2)[,c(1:3)] %>%
        setNames(c("date", "dengue", "sintomas_dengue")) %>%
        filter(date <= "2024-06-16" & date >= "2024-01-01") %>%
        mutate(
          across(
            .cols = -date,
            .fns = ~ as.numeric(ifelse(. == "<1", "0.5", .))
          )
        ) %>% 
        select("dengue", "sintomas_dengue") %>%
        as.matrix()
```


```{r}
print(result_out_list[[18]])
read.csv(file.path(path_source_denguetracker_GT,202425, paste0("RJ","_trends.csv")), 
               stringsAsFactors = FALSE, skip = 2)[,c(1:3)] %>%
        setNames(c("date", "dengue", "sintomas_dengue")) %>%
        filter(date <= "2024-06-16" & date >= "2024-04-09") %>%
        mutate(
          across(
            .cols = -date,
            .fns = ~ as.numeric(ifelse(. == "<1", "0.5", .))
          )
        ) %>% 
        select("dengue", "sintomas_dengue") %>%
        as.matrix()
```


```{r}
error_metrics <- function(predicted, actual) {
  # same length
  stopifnot(length(predicted) == length(actual))
  
  # RMSE (Root Mean Squared Error)
  rmse <- sqrt(mean((predicted - actual)^2))
  
  # RMSPE (Root Mean Squared Percentage Error)
  rmspe <- sqrt(mean(((predicted - actual)/actual)^2)) * 100
  
  # MAE (Mean Absolute Error)
  mae <- mean(abs(predicted - actual))
  
  # MAPE (Mean Absolute Percentage Error)
  mape <- mean(abs((predicted - actual)/actual)) * 100

  list(RMSE = rmse,
       RMSPE = rmspe,
       MAE = mae,
       MAPE = mape)
}

```


```{r}
# metrics
rmse <- calculate_metrics(result_out_list, "RMSE")

mae <- calculate_metrics(result_out_list, "MAE")

rmspe <- calculate_metrics(result_out_list, "RMSPE")

mape <- calculate_metrics(result_out_list, "MAPE")

mciw <- calculate_metrics(result_out_list, "MCIW")

mcr <- calculate_metrics(result_out_list, "MCR")

metrics_list <- list(rmse = rmse, mae = mae, rmspe = rmspe, mape = mape,
                     mciw = mciw, mcr = mcr)


brazil_states_full <- c(
  "Acre", "Alagoas", "Amapá", "Amazonas", "Bahia", "Ceará", "Distrito Federal", 
  "Espírito Santo", "Goiás", "Maranhão", "Mato Grosso", "Mato Grosso do Sul", 
  "Minas Gerais", "Pará", "Paraíba", "Paraná", "Pernambuco", "Piauí", 
  "Rio de Janeiro", "Rio Grande do Norte", "Rio Grande do Sul", "Rondônia", 
  "Roraima", "Santa Catarina", "São Paulo", "Sergipe", "Tocantins"
)

captions <- c("RMSE Results by State", "MAE Results by State",
              "RMSPE Results by State", "MAPE Results by State",
              "MCIW Results by State", "MCR Results by State")


for (i in 1:5) {
  cat(df_to_latex(metrics_list[[i]], 
                  caption = captions[i], 
                  label = paste0("tab:",names(metrics_list)[i]), 
                  highlight_mode = "min", 
                  highlight_n = 2))
}

cat(df_to_latex(metrics_list[[6]], 
                  caption = captions[6], 
                  label = paste0("tab:",names(metrics_list)[6]), 
                  highlight_mode = "max", 
                  highlight_n = 2))
```

```{r}
save(out_b_constant_FR_multi, file = file.path(data_save_path, "FR_b_constant_multi.RData"))
```

