...
#   "RS"
# )       
# E_vec <- exposure$exposure[c(20)]  

#all_states <- c("SP")
#E_vec <-  exposure$exposure[24]
...
# 定义 eval_lab（这里只用 eval_t，如果想要 "Initial/Peak..." 自己配个向量）
...
application_plot <- ggplot(df_plot, aes(x = pred_weeks)) +
  # ribbons
  geom_ribbon(
...
y_limits <- tibble::tibble(
  state = c("AL","BA","SP"),
  y_min = c(0,    0,     0),
  y_max = c(2500, 30000, 300000)
)

# 按你的 df_plot 继续画（保持原有 geoms / facet）
p <- ggplot(df_plot, aes(x = pred_weeks)) +
  ...
  theme_bw(base_size = 10) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),  # x 标签 45°
    legend.position      = c(0.15, 0.995),
    legend.justification = c(1, 1),
    legend.background    = element_rect(fill = scales::alpha("white", 0.75), color = NA),
    legend.key.height    = unit(0.25, "cm"),
...
# 关键：按州单独设定 y 轴 limits（不改数据）
p <- p + ggh4x::facetted_pos_scales(
  y = list(
    state == "AL" ~ scale_y_continuous(limits = c(y_limits$y_min[y_limits$state=="AL"],
                                                  y_limits$y_max[y_limits$state=="AL"])),
    state == "BA" ~ scale_y_continuous(limits = c(y_limits$y_min[y_limits$state=="BA"],
                                                  y_limits$y_max[y_limits$state=="BA"])),
    state == "SP" ~ scale_y_continuous(limits = c(y_limits$y_min[y_limits$state=="SP"],
                                                  y_limits$y_max[y_limits$state=="SP"]))
  )
)
