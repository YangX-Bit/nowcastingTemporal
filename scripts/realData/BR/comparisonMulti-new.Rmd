...
```{r}
pop_br <- readxl::read_xlsx(file.path(path_proj, "PAPER", "population_br_per_states_24to25.xlsx"))[-c(1,2,10,20,25,29),]

# expected number per week per state
expected <- case_df %>%
  mutate(
    start_date = as.Date("2024-03-03"),
    end_date   = case_when(
      uf %in% c("AL", "AP", "RR") ~ as.Date("2024-11-10"),
      uf %in% c("PE", "SE")       ~ as.Date("2024-09-29"),
      TRUE                        ~ as.Date("2024-07-28")
    )
  ) %>%
...
expected_within <- expected %>%
  group_by(uf) %>%
  mutate(E_it = mean(cases, na.rm = TRUE),   # Expected is the average here, since pop keeps the same
         SIR_it = cases / E_it) %>%
  ungroup()

exposure <- expected_within %>% select(uf, E_it) %>% unique()
...
#   "SC", "SP"
# )       
# E_vec <- exposure$E_it[c(15,20,23,24)]  
 
   all_states <- c(
"PR"
 )       
 E_vec <- exposure$E_it[c(15)] 
  ...
  suppressWarnings(
      for (i in seq_along(all_states)) {
        ...
        results_wGT_i <- run_moving_window(
          ...
          chains              = chain_num,
          refresh             = refresh,
          posterior_draws_path = file.path(file.path(dirname(dirname(path_proj)),"projects","draws","BR_indirE"))
        )
        saveRDS(results_wGT_i, file.path(fit_dir, "proposed_indir_E" ,sprintf("%s_ARABMwGT_fit.rds", state_i)))
      }
    )
  )
...
suppressMessages(
  suppressWarnings(
    ...
    for (i in seq_along(all_states)){
      ...
      results_wGT_i <- readRDS(file.path(fit_dir, "proposed_indir_E" ,sprintf("%s_ARABMwGT_fit.rds", state_i)))
