...
line_data <- data.frame(
  ...
  vline_date = as.Date(c("2024-07-28", "2024-11-17", "2024-11-17", "2024-07-28", "2024-07-28", "2024-07-28", 
                         "2024-07-28", "2024-07-28", "2024-07-28", "2024-07-28", "2024-07-28", "2024-07-28",
                         "2024-07-28", "2024-07-28", "2024-07-28", "2024-09-29", "2024-07-28", "2024-07-28", 
                         "2024-07-28", "2024-07-28", "2024-07-28", "2024-11-17", "2024-07-28", "2024-07-28",
                         "2024-09-29", "2024-07-28"))
)

case_df <- case_df %>% 
  filter(uf != "ES") %>%
  mutate(uf = factor(uf, levels = brazil_ufs))
line_data <- line_data %>%
  mutate(uf = factor(uf, levels = brazil_ufs))
...
print(actual_cases)
ggsave(filename = file.path(path_proj, "plots_to_show", "BR", "actual_cases_diff_lines.png"),
       plot = actual_cases,
       width = 10, height = 8, dpi = 300)


actual_cases_2 <- ggplot(case_df, aes(x = date, y = cases)) +
  geom_line(color = "steelblue", size = 0.8) +
...
```{r, message=FALSE}
set.seed(1)
## 1) Define once (before the loop)
state_end_map <- c(AL = 202447L, AP = 202447L, RR = 202447L,   # Nov 17, 2024
                   PE = 202440L, SE = 202440L)                 # Sep 29, 2024
...
suppressMessages(
  suppressWarnings(
      for (i in seq_along(all_states)) {
        ...
        end_i <- if (state_i %in% names(state_end_map)) state_end_map[[state_i]] else ew_end
        cat(sprintf(">>> [MCMC %d/%d] State=%s | cutoff EW=%s ...\n", 
                    i, length(all_states), state_i, end_i))
  ...
  suppressWarnings(
    ...
    for (i in seq_along(all_states)){
      ...
      for (j in 1:length(results_i)) {
          ...
          # draws
          pred <- get_N_lastL_draws(results_i[[j]]$fit_object$output_files(), num_N, min(num_N, L), if_impute = T)
          pred_wGT <- get_N_lastL_draws(results_wGT_i[[j]]$fit_object$output_files(), num_N_wGT, min(num_N, L), if_impute = T) # num_N is always smaller
          ...
          # crps
          tmp_crps[j] <- mean(scoringRules::crps_sample(y = y_true$sum_of_cases_final, dat = pred))
          tmp_wGT_crps[j]  <- mean(scoringRules::crps_sample(y = y_true$sum_of_cases_final, dat = pred_wGT))
...
for (i in seq_along(all_states)) {
  ...
  results_wGT_i<- readRDS(file.path(fit_dir, sprintf("%s_ARABMwGT_fit.rds", state_i)))
  # results
  arabm_i     <- extract_N_summary(results_i,    num_N = 4) # 默认名：ARABM, ARABM_lwr, ARABM_upr
  ...
  result_out_cut <- result_out_i %>%
    ...
    ungroup()
  results_list[[state_i]] <- result_out_cut
...
final_metrics_out <- final_metrics %>% filter(model == "ARABM" | model == "ARABMwGT") 
...
# write
write_xlsx(final_metrics, file.path(path_proj, "plots_to_show","BR","final_metrics_with_diff_ends.xlsx"))

write_xlsx(final_metrics_out, file.path(path_proj, "plots_to_show","BR","final_metrics_with_diff_ends_2m.xlsx"))
```

```{r}
# states_in_paper <- c("AM", "RJ", "RN")
states_in_paper <- NULL
...
results_paper <- final_metrics_out %>% filter(state %in% states_in_paper)
results_sm <- final_metrics_out %>% filter(!(state %in% states_in_paper))

metrics <- c("RMSE", "RMSPE", "crps", "mcr95", "mcr50")
df_paper <- highlight_metric_pairs_simple(
      df_wide    = results_paper,
      metrics    = metrics,
...
cat(render_latex_metric_pairs(
      df_print = df_paper,
      caption  = paste0("Baseline vs Proposed across metrics (", scenario = scenarios[j], "; last ", 1  ," weeks)")
))

```

```{r}
df_sm <- highlight_metric_pairs_simple(
      df_wide    = results_sm,
      metrics    = metrics,
      directions = c(RMSE="lower", RMSPE="lower", mcr95="closer", mcr50="closer", crps="lower"),
      targets    = c(CR95=0.95, CR50=0.50),  # or 95/50 if your data are percentages
...
# 定义数据
df <- data.frame(
  type = c("Historical data (no snapshots)", "Runoff triangle snapshots"),
  start = as.Date(c("2022-03-10", "2024-03-10")),
  end   = as.Date(c("2024-03-10", "2025-05-11"))
)
...
ggplot(df) +
  ...
  labs(title = "Timeline of Dengue Surveillance Data",
       x = "Date", y = "") +
