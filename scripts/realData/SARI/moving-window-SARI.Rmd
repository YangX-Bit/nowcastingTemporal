...
models_to_use <- c("baseline", "baseline2", "proposed")
```
...
SARI_collapsed <- SARI %>%
  ...
  summarise(across(-regionalsaude, ~ sum(.x, na.rm = TRUE)), .groups = "drop") %>% # combine
...
save(out_SARI, file = file.path(path_proj, "data", "results", "SARI","SARI_090802_to_901122_D=26.RData"))
# load( file.path(path_proj, "data", "results", "SARI","SARI_090802_to_901122_D=15.RData"))
...
for (i in 1:length(out_SARI$dates)) {
  num_N <- ncol(out_SARI$baseline2[[i]]$draws(variables = "N", format = "draws_matrix"))
  num_N_hist <- ncol(out_SARI$proposed[[i]]$draws(variables = "N", format = "draws_matrix"))
  
  pred <- get_N_lastL_draws(out_SARI$baseline2[[i]]$output_files(), num_N, min(num_N, L), if_impute = T)
  pred_hist <- get_N_lastL_draws(out_SARI$proposed[[i]]$output_files(), num_N, min(num_N, L), if_impute = T)
...
for (i in 1:length(out_SARI$dates)) {
  crps_sari <- as.numeric(c(NA, tmp_crps[[i]], tmp_hist_crps[[i]]))
  mcr95_sari <- as.numeric(c(NA, tmp_mcr95[[i]], tmp_hist_mcr95[[i]]))
  mcr50_sari <- as.numeric(c(NA, tmp_mcr50[[i]], tmp_hist_mcr50[[i]]))
...
#
final_metrics_out <- bind_rows(final_metrics) %>%  filter(model != "baseline")
...
# write
write_xlsx(final_metrics_out, file.path(path_proj, "plots_to_show","SARI","final_metrics_D26.xlsx"))
...
"
  ...
  for (i in seq_len(nrow(df))) {
    ...
    hl <- function(b, p, smaller_is_better = TRUE) {
    ...
    vals <- c(
      hl(row$RMSE_baseline2,  row$RMSE_proposed,  smaller_is_better = TRUE),
      hl(row$RMSPE_baseline2, row$RMSPE_proposed, smaller_is_better = TRUE),
      hl(row$crps_baseline2,  row$crps_proposed,  smaller_is_better = TRUE),
      hl(row$mcr95_baseline2, row$mcr95_proposed, smaller_is_better = FALSE),
      hl(row$mcr50_baseline2, row$mcr50_proposed, smaller_is_better = FALSE)
    )
