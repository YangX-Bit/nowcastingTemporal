...
```{r}
  ...
  tau_x <- 30      # information level for x
  tau_s <- 50         # for s
  ...
  beta_s    <- rnorm(2 * rep(1:H_used, each = 2), mean = rev(rep(1:H_used, each = 2)), sd = sigma_beta)
  
  #beta_s <- rev(c(3,3,2,2,1,1))
  s_t <- as.vector(fourier_basis_scaled[,c(1:(2*H_used))] %*% beta_s)

    s_t_2 <- as.vector(fourier_basis[,c(1:(2*H_used))] %*% beta_s)
...
```{r}
# GT
y_true_l <- tail(b_ou_FR$case_true[1:ind[test_num]], last_len)

y_rep_mat <- test[[test_num]]$draws(variables = "N", format = "draws_matrix")
pred_mat_l <- tail(t(y_rep_mat), last_len)

crps_vec_l  <- crps_sample(y   = y_true_l,  
                         dat = pred_mat_l)
mean_crps_l <- mean(crps_vec_l)

#
y_rep_mat_GT <- test_GT[[test_num]]$draws(variables = "N", format = "draws_matrix")
pred_mat_GT_l <- tail(t(y_rep_mat_GT), last_len) 

crps_vec_GT_l  <- crps_sample(y   = y_true_l,  
                         dat = pred_mat_GT_l)
mean_crps_GT_l <- mean(crps_vec_GT_l)
...
mcmc_areas(test_GT[[test_num]]$draws("beta0"), prob = 0.95, prob_outer = 0.95)
