...
all_cr50_NFRM <- vector("list", length(last_lens))
all_cr95_NFRM <- vector("list", length(last_lens))
names(all_cr95_NFRM) <- names(all_cr50_NFRM) <- names(all_crps_NFRM)
...
final_cr50_tbl_NFRM <- do.call(rbind, all_cr50_NFRM)
final_cr95_tbl_NFRM <- do.call(rbind, all_cr95_NFRM)
...
preds_result_NFRM <- readRDS(file.path(path_to_save_NFRS, "final_NFRM_predictions.rds"))
```
