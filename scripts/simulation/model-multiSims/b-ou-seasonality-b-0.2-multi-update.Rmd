...
for (i in 1:num_sims) {
  ...
  full_rep_num_NFRM <- full_rep_num_NFRM + sum(data_NFRM[[i]]$q[,16] > 0.95)
}
full_rep_ratio_NFRM <- round(full_rep_num_NFRM/(length(data_NFRM[[i]]$q[,16] ) * num_sims),4) * 100
full_rep_ratio_NFRM
...
files_bsl_NFRM <- map(results_bsl_NFRM, "files")
# 4) Result (adress)
...
preds_bsl_D_NFRM  <- vector("list", total_runs)
preds_prps_D_NFRM <- vector("list", total_runs)
...
for (i in seq_along(n_weeks)) {
  ...
  for (run in seq_len(num_sims)) {
    ...
    # read last Dth
    preds_bsl_D_NFRM[[idx]]  <- get_N_lastL_mean(files_bsl_NFRM[[idx]],  wks, D)
    preds_prps_D_NFRM[[idx]] <- get_N_lastL_mean(files_prps_NFRM[[idx]], wks, D)
    
    # read last Dth
    actual_D[[idx]]     <- tail(data_NFRM[[run]]$case_true[1:wks], D)
  }
}
...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    for (run in seq_len(num_sims)) {
      ...
      # tail the saved data
      pred_bsl_L  <- tail(preds_bsl_D_NFRM[[idx]],  L)
      pred_prps_L <- tail(preds_prps_D_NFRM[[idx]], L)
      actual_L    <- tail(actual_D[[idx]],     L)
      
      metrics_bsl_NFRM[[i]][[run]]  <- error_metrics(pred_bsl_L,  actual_L)
      metrics_prps_NFRM[[i]][[run]] <- error_metrics(pred_prps_L, actual_L)
    }
  }
  ...
  for (i in seq_along(n_weeks)) {
    prps_list <- metrics_prps_NFRM[[i]]
    bsl_list  <- metrics_bsl_NFRM[[i]]
...
bsl_has_na_NFRM  <- vapply(preds_bsl_D_NFRM,  anyNA, logical(1))
prps_has_na_NFRM <- vapply(preds_prps_D_NFRM, anyNA, logical(1))  

data.frame(
  model    = c("bsl", "prps"),
  total    = c(length(preds_bsl_D_NFRM), length(preds_prps_D_NFRM)),
  with_NA  = c(sum(bsl_has_na_NFRM), sum(prps_has_na_NFRM)),
  na_prop  = c(mean(bsl_has_na_NFRM), mean(prps_has_na_NFRM))
)
...
for (r in seq_len(nrow(idx_df))) {
  ...
  cache_prps[[r]] <- get_last_maxL_draws(files_prps_NFRM[[id]], wks, maxL)
  cache_bsl[[r]]  <- get_last_maxL_draws(files_bsl_NFRM[[id]],  wks, maxL)
}
...
final_cr50_tbl_NFRM <- do.call(rbind, all_cr50)
final_cr95_tbl_NFRM <- do.call(rbind, all_cr95)
final_NFRM <- rbind(final_crps_tbl_NFRM, final_cr50_tbl_NFRM, final_cr95_tbl_NFRM)
rownames(final_NFRM) <- NULL
print(final_NFRM)
...
# saveRDS(final_NFRM, file = file.path(path_to_save_NFRM, "final_NFRM.rds"))
# prediction results
# saveRDS(preds_result_NFRM, file = file.path(path_to_save_NFRM, "final_NFRM_predictions.rds"))
```


```{r}
data_NFRM <- readRDS(file.path(path_to_save_NFRM, "data_NFRM.rds"))
...
final_NFRM <- readRDS(file.path(path_to_save_NFRM, "final_NFRM.rds"))

preds_result_NFRM <- readRDS(file.path(path_to_save_NFRM, "final_NFRM_predictions.rds"))
```
