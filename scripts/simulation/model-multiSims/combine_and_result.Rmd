...
# ---- plot (6 rows x 5 cols: (scenario × model) by eval_t) ----
sims_result_plot <- ggplot(df_plot, aes(x = week)) +
  # ribbons for the chosen CI (group by scenario/model/panel)
  geom_ribbon(
  ...
  # posterior means (no legend for models)
  geom_line(
    aes(y = mean, color = model,
        group = interaction(scenario, model, eval_lab)),
    linewidth = rel(0.5), show.legend = FALSE, na.rm = TRUE
  ) +
  # reported overlay (line + points) — legend ON; group by panel
  geom_line(
    aes(y = reported, color = "Reported cases",
        group = eval_lab),
    linewidth = rel(0.4), show.legend = TRUE, na.rm = TRUE
  ) +
  geom_point(
    aes(y = reported, color = "Reported cases",
        group = eval_lab),
    size = rel(0.4), show.legend = TRUE, na.rm = TRUE
  ) +
  # truth overlay — legend ON; group by panel
  geom_line(
    aes(y = truth, color = "Actual cases",
        group = eval_lab),
    linewidth = rel(0.4), linetype = 6, show.legend = TRUE, na.rm = TRUE
  ) +
  facet_grid(
...
ggsave(filename = file.path(path_proj, "plots_to_show", "simulation", "results.png"),
