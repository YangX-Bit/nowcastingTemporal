...
df_plot <- preds_result %>%
  filter(run == run_to_plot, scenario %in% c("FR","MNFR","SNFR")) %>%
  mutate(
    scenario = factor(scenario, levels = c("FR","MNFR","SNFR")),
    model    = factor(model, levels = c("Baseline","Proposed"))
  )

# enforce left-to-right time order in facets (e.g., 53,66,79,92,104)
# define ordered time points and labels
eval_times    <- sort(unique(df_plot$eval_t))
stage_labels  <- c("Initial", "Increasing", "Peak", "Decreasing", "Finish")

# safety check
stopifnot(length(eval_times) == length(stage_labels))

# make label vector like "t = 53 (Initial)"
eval_labels <- paste0("t = ", eval_times, " (", stage_labels, ")")

# apply as factor
df_plot <- df_plot %>%
  mutate(
    eval_lab = factor(
      paste0("t = ", eval_t, " (", stage_labels[match(eval_t, eval_times)], ")"),
      levels = eval_labels  # ensure correct order in facet
    )
  )
...
# Realization of seasonality
```{r}
# centralized 
s_t_centered <- s_t - mean(s_t)
S_t <- exp(s_t_centered)
...
p_s <- ggplot(df, aes(x = Week, y = Seasonality)) +
  geom_line(size = 1, color = "black") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "black") +
  scale_y_continuous(
    name = "Seasonality multiplier",
    limits = c(min(S_t) * 0.9, max(S_t) * 1.1),
    expand = c(0, 0)
  ) +
  scale_x_continuous(
    name = "Week",
    breaks = seq(0, T, by = 13), # 按季度标
    expand = c(0, 0)
  ) +
...
print(p_s)
ggsave(filename = file.path(path_proj, "plots_to_show", "simulation", "sims-seasonality.png"),
       plot = p_s,
