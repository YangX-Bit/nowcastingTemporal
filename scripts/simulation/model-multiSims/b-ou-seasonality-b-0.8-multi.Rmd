...
post_draws_path = file.path(path_proj, "data", "posterior")
...
final_tbl <- do.call(rbind, all_results)
row.names(final_tbl) <- NULL

final_long <- final_tbl %>%
  pivot_longer(
...
final_crps_tbl <- do.call(rbind, all_crps)
rownames(final_crps_tbl) <- NULL
