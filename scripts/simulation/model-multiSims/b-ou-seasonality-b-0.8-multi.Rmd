...
results_bsl_FR <- pmap(param_tbl, function(n_weeks, run) {
...
# 4) Result
fit_bsl_FR     <- map(results_bsl_FR, "fit")
varnames_bsl_FR <- map(results_bsl_FR, "varnames")
N_bsl_FR  <- map(results_bsl_FR, "N")

toc(log = TRUE) 
```

```{r}
tic("FR proposed model")
hypers = hypers_q(phi_ref = 0.1, D_ref = 25, type = "exponential", alpha_phi = 1.4, sd_log_b = 1, delay_seq = 0:15)

# run everything
results_prps_FR <- pmap(param_tbl, function(n_weeks, run) {
  message("Simulating: ", n_weeks, " weeks — run ", run)
...
# 4) Result
fit_prps_FR     <- map(results_prps_FR, "fit")
varnames_prps_FR <- map(results_prps_FR, "varnames")
N_prps_FR  <- map(results_prps_FR, "N")
...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      idx       <- (run - 1) * n_w + i
      pred_bsl  <- tail(N_bsl_FR[[idx]],  L)
      pred_prps <- tail(N_prps_FR[[idx]], L)
...
final_tbl_FR <- do.call(rbind, all_results)
...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      ...
      pred_prps <- t(fit_prps_FR[[idx]]$draws("N", format = "draws_matrix"))
      tmp_prps[run] <- mean(crps_sample(y = y_true, dat = pred_prps[(wks-L+1):wks, ]))
      
      # baseline model
      pred_bsl <- t(fit_bsl_FR[[idx]]$draws("N", format = "draws_matrix"))
