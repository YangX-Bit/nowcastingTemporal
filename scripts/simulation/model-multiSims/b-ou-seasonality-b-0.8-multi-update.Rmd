...
post_draws_path_FR = file.path(file.path(dirname(dirname(path_proj)),"projects","draws","FR"))

source(file.path(path_proj, "source", "functions", "simulation_functions.R"))
source(file.path(path_proj, "source", "functions", "metrics.R"))
...
#n_weeks   <-  c(52)
lambda_seq_FR <- matrix(NA, ncol = T_len, nrow = num_sims)
...
### Simulation - fully reported - Moderate
...
for (j in 1:num_sims) {
  data_FR[[j]] <- simulateData(
    params = list(
      data = list(
        lambda = lambda_seq_FR[j,],
        T       = T_len,
      ...
      q_model = list(
        method        = "b_ou",
        method_params = list(theta_logb = 0.7, mu_logb = log(0.7), init_logb = log(0.7), sigma_logb = 0.4,
                             theta_logitphi = 0.7, 
...
par(mfrow = c(2, 3))
for (i in 1:num_sims) {
  matplot(t(data_FR[[i]]$q), type = "l", lty = 1, ylim = c(0, 1))
  full_rep_num_FR <- full_rep_num_FR + sum(data_FR[[i]]$q[,16] > 0.95)
}
full_rep_ratio_FR <- round(full_rep_num_FR/(length(data_FR[[i]]$q[,16] ) * num_sims),4) * 100
full_rep_ratio_FR
...
results_bsl_FR <- pmap(param_tbl_FR, function(n_weeks, run) {
  ...
  stan_data <- c(
    list(
      T = n_weeks,
      D = D,
      Y = data_FR[[run]]$case_reported_cumulated[1:n_weeks, ]
    ),
    hypers
  )
suppressMessages(
...
# 4) Result (adress)
files_bsl_FR <- map(results_bsl_FR, "files")
...
# run everything
results_prps_FR <- pmap(param_tbl_FR, function(n_weeks, run) {
  message("Simulating: ", n_weeks, " weeks — run ", run)
...
# 4) Result
files_prps_FR <- map(results_prps_FR, "files")
...
preds_bsl_D_FR  <- vector("list", total_runs)
preds_prps_D_FR <- vector("list", total_runs)
...
for (i in seq_along(n_weeks)) {
  ...
  for (run in seq_len(num_sims)) {
    ...
    # read last Dth
    preds_bsl_D_FR[[idx]]  <- get_N_lastL_mean(files_bsl_FR[[idx]],  wks, D)
    preds_prps_D_FR[[idx]] <- get_N_lastL_mean(files_prps_FR[[idx]], wks, D)
    
    # read last Dth
    actual_D[[idx]]     <- tail(data_FR[[run]]$case_true[1:wks], D)
  }
}

# --- 3) Calculate according to different L --------------------------------------
all_results_FR <- vector("list", length(last_lens))
names(all_results_FR) <- paste0("last_", last_lens)

for (li in seq_along(last_lens)) {
  L <- last_lens[li]
  
  # for each time t and simulations
  metrics_prps_FR <- replicate(n_w, vector("list", num_sims), simplify = FALSE)
  metrics_bsl_FR  <- replicate(n_w, vector("list", num_sims), simplify = FALSE)
  ...
  for (i in seq_along(n_weeks)) {
    for (run in seq_len(num_sims)) {
      ...
      # tail the saved data
      pred_bsl_L  <- tail(preds_bsl_D_FR[[idx]],  L)
      pred_prps_L <- tail(preds_prps_D_FR[[idx]], L)
      actual_L    <- tail(actual_D[[idx]],     L)
      
      metrics_bsl_FR[[i]][[run]]  <- error_metrics(pred_bsl_L,  actual_L)
      metrics_prps_FR[[i]][[run]] <- error_metrics(pred_prps_L, actual_L)
    }
  }
  ...
  for (i in seq_along(n_weeks)) {
    prps_list <- metrics_prps_FR[[i]]
    bsl_list  <- metrics_bsl_FR[[i]]
  ...
  all_results_FR[[li]] <- out
}

final_tbl_FR <- do.call(rbind, all_results_FR)
row.names(final_tbl_FR) <- NULL

final_long_FR <- final_tbl_FR %>%
  tidyr::pivot_longer(
...
bsl_has_na_FR  <- vapply(preds_bsl_D_FR,  anyNA, logical(1))
prps_has_na_FR <- vapply(preds_prps_D_FR, anyNA, logical(1))  

data.frame(
  model    = c("bsl", "prps"),
  total    = c(length(preds_bsl_D_FR), length(preds_prps_D_FR)),
  with_NA  = c(sum(bsl_has_na_FR), sum(prps_has_na_FR)),
  na_prop  = c(mean(bsl_has_na_FR), mean(prps_has_na_FR))
)
```
...
```{r}
all_crps_FR <- vector("list", length(last_lens))
names(all_crps_FR) <- paste0("last_", last_lens)

# NEW: containers for coverage
all_cr50_FR <- vector("list", length(last_lens))
all_cr95_FR <- vector("list", length(last_lens))
names(all_cr95_FR) <- names(all_cr50_FR) <- names(all_crps_FR)
...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      ...
      y_true <- tail(data_FR[[run]]$case_true[1:wks], L)
      
      pred_prps <- get_N_lastL_draws(files_prps_FR[[idx]], wks, L)
      pred_bsl  <- get_N_lastL_draws(files_bsl_FR[[idx]],  wks, L)
...
final_crps_tbl_FR  <- do.call(rbind, all_crps_FR)
final_cr50_tbl_FR <- do.call(rbind, all_cr50_FR)
final_cr95_tbl_FR <- do.call(rbind, all_cr95_FR)

# Optionally combine:
final_FR <- rbind(final_crps_tbl_FR, final_cr50_tbl_FR, final_cr95_tbl_FR)
rownames(final_FR) <- NULL
print(final_FR)
...
for (r in seq_len(nrow(idx_df))) {
  ...
  cache_prps[[r]] <- get_last_maxL_draws(files_prps_FR[[id]], wks, maxL)
  cache_bsl[[r]]  <- get_last_maxL_draws(files_bsl_FR[[id]],  wks, maxL)
}
...
final_crps_tbl_FR <- do.call(rbind, all_crps)
final_cr50_tbl_FR <- do.call(rbind, all_cr50)
final_cr95_tbl_FR <- do.call(rbind, all_cr95)
final_FR <- rbind(final_crps_tbl_FR, final_cr50_tbl_FR, final_cr95_tbl_FR)
rownames(final_FR) <- NULL
print(final_FR)
...
```{r}
path_to_save_FR <- file.path(path_proj, "data", "simulation","results","FR")

# sim data
# saveRDS(data_FR, file = file.path(path_to_save_FR, "data_FR.rds"))
# path
# saveRDS(files_prps_FR, file = file.path(path_to_save_FR, "files_prps_FR.rds"))
# saveRDS(files_bsl_FR,  file = file.path(path_to_save_FR, "files_bsl_FR.rds"))
# metrics
# saveRDS(final_long_FR, file = file.path(path_to_save_FR, "final_long_FR.rds"))
# saveRDS(final_FR, file = file.path(path_to_save_FR, "final_FR.rds"))
# prediction results
# saveRDS(preds_result_FR, file = file.path(path_to_save_FR, "final_FR_predictions.rds"))
```


```{r}
data_FR <- readRDS(file.path(path_to_save_FR, "data_FR.rds"))
files_prps_FR <- readRDS(file.path(path_to_save_FR, "files_prps_FR.rds"))
files_bsl_FR  <- readRDS(file.path(path_to_save_FR, "files_bsl_FR.rds"))

# result table
final_long_FR     <- readRDS(file.path(path_to_save_FR, "final_long_FR.rds"))
final_FR <- readRDS(file.path(path_to_save_FR, "final_FR.rds"))

preds_result_FR <- readRDS(file.path(path_to_save_FR, "final_FR_predictions.rds"))
```
