...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      ...
      pred_bsl  <- get_N_lastL_mean(files_bsl_FR[[idx]],  wks, L)
      pred_prps <- get_N_lastL_mean(files_prps_FR[[idx]], wks, L)
      actual    <- tail(data_FR[[run]]$case_true[1:wks], L)

      metrics_bsl_FR[[i]][[run]]  <- error_metrics(pred_bsl,  actual)
      metrics_prps_FR[[i]][[run]] <- error_metrics(pred_prps, actual)
    }
  }
  ...
  for (i in seq_along(n_weeks)) {
    prps_list_FR <- metrics_prps_FR[[i]]
    bsl_list_FR  <- metrics_bsl_FR[[i]]

    out$prps_RMSE[i]  <- mean(sapply(prps_list_FR, `[[`, "RMSE"))
    out$prps_RMSPE[i] <- mean(sapply(prps_list_FR, `[[`, "RMSPE"))
    out$prps_MAE[i]   <- mean(sapply(prps_list_FR, `[[`, "MAE"))
    out$prps_MAPE[i]  <- mean(sapply(prps_list_FR, `[[`, "MAPE"))

    out$bsl_RMSE[i]   <- mean(sapply(bsl_list_FR,  `[[`, "RMSE"))
    out$bsl_RMSPE[i]  <- mean(sapply(bsl_list_FR,  `[[`, "RMSPE"))
    out$bsl_MAE[i]    <- mean(sapply(bsl_list_FR,  `[[`, "MAE"))
    out$bsl_MAPE[i]   <- mean(sapply(bsl_list_FR,  `[[`, "MAPE"))
  }
