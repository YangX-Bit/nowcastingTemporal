...
### Simulation - Not fully reported - Severe
...
# 4) Result
fit_bsl_NFRS     <- map(results_bsl_NFRS, "fit")
varnames_bsl_NFRS <- map(results_bsl_NFRS, "varnames")
N_bsl_NFRS  <- map(results_bsl_NFRS, "N")
...
# 4) Result
fit_prps_NFRS     <- map(results_prps_NFRS, "fit")
varnames_prps_NFRS <- map(results_prps_NFRS, "varnames")
N_prps_NFRS  <- map(results_prps_NFRS, "N")
...
for (li in seq_along(last_lens)) {
  ...
  metrics_prps_NFRS <- vector("list", n_w)
  metrics_bsl_NFRS  <- vector("list", n_w)
  for (i in seq_len(n_w)) {
    metrics_prps_NFRS[[i]] <- vector("list", num_sims)
    metrics_bsl_NFRS[[i]]  <- vector("list", num_sims)
  }
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      idx       <- (run - 1) * n_w + i
      pred_bsl  <- tail(N_bsl_NFRS[[idx]],  L)
      pred_prps <- tail(N_prps_NFRS[[idx]], L)
      actual    <- tail(data_NFRS[[run]]$case_true[1:wks], L)
      
      metrics_bsl_NFRS[[i]][[run]]  <- error_metrics(pred_bsl,  actual)
      metrics_prps_NFRS[[i]][[run]] <- error_metrics(pred_prps, actual)
    }
  }
  ...
  for (i in seq_along(n_weeks)) {
    prps_list_NFRS <- metrics_prps_NFRS[[i]]
    bsl_list_NFRS  <- metrics_bsl_NFRS[[i]]
    
    out$prps_RMSE[i]  <- mean(sapply(prps_list_NFRS, `[[`, "RMSE"))
    out$prps_RMSPE[i] <- mean(sapply(prps_list_NFRS, `[[`, "RMSPE"))
    out$prps_MAE[i]   <- mean(sapply(prps_list_NFRS, `[[`, "MAE"))
    out$prps_MAPE[i]  <- mean(sapply(prps_list_NFRS, `[[`, "MAPE"))
    
    out$bsl_RMSE[i]   <- mean(sapply(bsl_list_NFRS,  `[[`, "RMSE"))
    out$bsl_RMSPE[i]  <- mean(sapply(bsl_list_NFRS,  `[[`, "RMSPE"))
    out$bsl_MAE[i]    <- mean(sapply(bsl_list_NFRS,  `[[`, "MAE"))
    out$bsl_MAPE[i]   <- mean(sapply(bsl_list_NFRS,  `[[`, "MAPE"))
  }
...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      ...
      pred_prps <- t(fit_prps_NFRS[[idx]]$draws("N", format = "draws_matrix"))
      tmp_prps[run] <- mean(crps_sample(y = y_true, dat = pred_prps[(wks-L+1):wks, ]))
      
      # baseline model
      pred_bsl <- t(fit_bsl_NFRS[[idx]]$draws("N", format = "draws_matrix"))
...
rownames(final_crps_tbl_NFRS) <- NULL
