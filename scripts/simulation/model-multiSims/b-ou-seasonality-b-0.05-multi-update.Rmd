...
for (i in 1:num_sims) {
  ...
  full_rep_num_NFRS <- full_rep_num_NFRS + sum(data_NFRS[[i]]$q[,16] > 0.95)
}
full_rep_ratio_NFRS <- round(full_rep_num_NFRS/(length(data_NFRS[[i]]$q[,16] ) * num_sims),4) * 100
full_rep_ratio_NFRS
...
```{r}
data_NFRS <- readRDS(file.path(path_to_save_NFRS, "data_NFRS.rds"))
