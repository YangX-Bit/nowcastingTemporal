...
results_bsl_NFRS <- pmap(param_tbl_NFRS, function(n_weeks, run) {
  ...
  stan_data <- c(
    list(
      T = n_weeks,
      D = D,
      Y = num_sims[[run]]$case_reported_cumulated[1:n_weeks, ]
    ),
    hypers
  )
suppressMessages(
...
for (li in seq_along(last_lens)) {
  ...
  for (i in seq_along(n_weeks)) {
    ...
    for (run in seq_len(num_sims)) {
      ...
      pred_bsl  <- get_N_lastL_mean(files_bsl_NFRS[[idx]],  wks, L)
      pred_prps <- get_N_lastL_mean(files_prps_NFRS[[idx]], wks, L)
...
saveRDS(final_crps_tbl_NFRS, file = file.path(path_to_save_NFRS, "final_crps_tbl_NFRS.rds"))
```
...
final_crps_tbl_NFRS <- readRDS(file.path(path_to_save_NFRS, "final_crps_tbl_NFRS.rds"))
