...
A <- sd_eps_emp * sqrt(2 * tau_s) # amplitude/sd
phi <- runif(1, 0, 2*pi)

# alpha gamma
alpha <- A*cos(phi); gamma <- A*sin(phi)

s_t <-vapply(t, function(tt) {
...
for (i in 1:length(ind)) {
  ...
  stan_data_GT <- c(list(T = ind[i], D = D, Y = b_ou_FR$case_reported_cumulated[1:ind[i], ]), hypers, 
               list(K = ncol(matrix(x_t)), x = as.matrix(matrix(x_t)[1:ind[i], 1]),
                    P = weeks_per_yr, H = 1, week = head(rep(1:weeks_per_yr, years), ind[i])
                    ))
...
mcmc_areas(test_GT[[test_num]]$draws("alpha"), prob = 0.95, prob_outer = 0.95)
mcmc_areas(test_GT[[test_num]]$draws("gamma"), prob = 0.95, prob_outer = 0.95)
...
param_true = tibble(
    parameter = grep("^s\\[.+\\]$", varnames_GT[[test_num]], value = TRUE),
    x = s_t[1:ind[test_num]]
)
mcmc_areas(test_GT[[test_num]]$draws("s"), prob_outer = 0.95) +
    geom_point(aes(x = x), param_true, color = "red", size = 1)
...
param_true = tibble(
    parameter = grep("^s\\[.+\\]$", varnames_GT_NFR[[test_num]], value = TRUE),
    x = s_t[1:ind[test_num]]
)
mcmc_areas(test_GT_NFR[[test_num]]$draws("s"), prob_outer = 0.95) +
    geom_point(aes(x = x), param_true, color = "red", size = 1)
```
