...
sigma_eps <- 0.2
epsilon <- rnorm(T_len, 0, sigma_eps)
var_eps_emp <- var(epsilon)

# --- (4) use tau to control the variance --------------------------------------------------
tau_x <- 5      # information level for x
tau_s <- 3      # for s

c_s <- sqrt(tau_s * var_eps_emp)
s_scaled <- c_s * s_t

# --- (5) choose prior for β (control the var of x part) --------------------------------------------------
sigma_beta <- sqrt(tau_x * var_eps_emp)
beta1 <- rnorm(T_len, 0.5, sigma_beta)       # β₁ ~ N(0, σ²_beta)
