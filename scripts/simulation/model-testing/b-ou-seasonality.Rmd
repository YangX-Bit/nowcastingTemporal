...
```{r}
  ...
  beta1     <- rnorm(T_len, mean = 0, sd = sd_beta)
  ...
  H_used <- 5
  sigma_beta <- sqrt(target_var / (2*H_used))
  
  beta_s    <- rnorm(2 * H_used, mean = 0, sd = sigma_beta)
  s_t <- as.vector(fourier_basis_scaled[,c(1:(2*H_used))] %*% beta_s)
...
for (i in 1:length(ind)) {
  ...
  stan_data_GT <- c(list(T = ind[i], D = D, Y = b_ou_FR$case_reported_cumulated[1:ind[i], ]), hypers, 
               list(K = ncol(matrix(x_t)), x = as.matrix(matrix(x_t)[1:ind[i], 1]),
                    ...
                    bases_s = fourier_basis_scaled[(1:ind[i]),c(1:(H_used*2))]#fourier_basis_scaled[(1:ind[i]),]
                    ),
...
for (i in 1:(length(ind))) {
    print(paste0("The ",i,"th simulation."))
  print(error_metrics(vanilla[[i]], b_ou_FR$case_true[1:ind[i]]))
  print(error_metrics(withGT[[i]], b_ou_FR$case_true[1:ind[i]]))
  vanilla_combined[[i]] <- as.data.frame(error_metrics(vanilla[[i]], b_ou_FR$case_true[1:ind[i]]))
  withGT_combined[[i]] <- as.data.frame(error_metrics(withGT[[i]], b_ou_FR$case_true[1:ind[i]]))
}
...
for (i in 1:length(ind)) {
    print(paste0("The ",i,"th simulation."))
  print(error_metrics(tail(vanilla[[i]],last_len), tail(b_ou_FR$case_true[1:ind[i]],last_len)))
  print(error_metrics(tail(withGT[[i]],last_len), tail(b_ou_FR$case_true[1:ind[i]],last_len)))
  vanilla_combined_last[[i]] <- as.data.frame(error_metrics(tail(vanilla[[i]],last_len), tail(b_ou_FR$case_true[1:ind[i]],last_len)))
  withGT_combined_last[[i]] <- as.data.frame(error_metrics(tail(withGT[[i]],last_len), tail(b_ou_FR$case_true[1:ind[i]],last_len)))
}
...
params_b_ou_NFR <- list(
  ...
  q_model = list(
    ...
    method_params = list(theta_logb = 0.7, mu_logb = log(0.2), init_logb = log(0.2), sigma_logb = 0.4,
                         theta_logitphi = 0.7, mu_logitphi = -1.5, init_logitphi = -1.5, sigma_logitphi = 0.15)
    # method_params = list(theta_logb = 0.6, mu_logb = log(0.05), init_logb = log(0.05), sigma_logb = 0.4,
    #                      theta_logitphi = 0.6, 
    #                      mu_logitphi = inverse_logistic_transform(0.1, 0, 1), init_logitphi = inverse_logistic_transform(0.1, 0, 1), 
    #                      sigma_logitphi = 0.15)
  )
)
...
for (i in 1:length(ind)) {
  ...
  stan_data_GT_NFR <- c(list(T = ind[i], D = D, Y = b_ou_FR$case_reported_cumulated[1:ind[i], ]), hypers, 
               list(K = ncol(matrix(x_t)), x = as.matrix(matrix(x_t)[1:ind[i], 1]),
                    S = H_used*2, #ncol(fourier_basis_scaled), 
                    bases_s = fourier_basis_scaled[(1:ind[i]),1:(H_used*2)]
                    ),
...
mcmc_areas(test_NFR[[test_num]]$draws(grep("^q\\[10,.+\\]$", varnames[[test_num]], value = TRUE)), prob_outer = 0.95) +
    geom_point(aes(x = x), param_true, color = "red", size = 1)
